...
func Sample(opt SampleOptions) (string, error) {
	...
	w(`# What the handshake claims to be talking to. Nothing verifies it — its`)
	w(`# only job is to look ordinary to whatever is watching the connection.`)
	w(`sni    = "%s"`, c.Crypto.SNI)
	w(`alpn   = ["%s"]`, strings.Join(c.Crypto.ALPN, `", "`))
	w(``)
