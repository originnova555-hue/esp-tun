...
// after the handler is running (no synchronisation). n <= 0 is ignored.
func SetPacketThreshold(n int64) {
	if n > 0 {
		packetThreshold = n
	}
}
...
func (h *sentPacketHandler) detectLostPackets(now monotime.Time, encLevel protocol.EncryptionLevel) {
	...
	for pn, p := range pnSpace.history.Packets() {
		...
		} else if int64(pnSpace.history.Difference(pnSpace.largestAcked, pn)) >= packetThreshold {
			packetLost = true
			if !p.isPathProbePacket && p.IsAckEliciting() {
				if h.logger.Debug() {
